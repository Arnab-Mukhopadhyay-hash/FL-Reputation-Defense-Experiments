...
class Config:
    ...
    # Hardware & System
    MAX_WORKERS = 2
    OUTPUT_DIR = "cifar10_v11_results"
    DEVICE_LIST = (
...
def compute_cluster_opinions(
    ...
    """
    NOVELTY: Adaptive Salient Feature Alignment (ASFA).
    
    1. Dynamic Thresholding: Uses statistical properties (Mean + Std Dev) of the 
       server reference to determine the 'saliency' threshold dynamically 
       rather than a fixed percentage.
    2. Adaptive Masking: Allows more features in high-variance scenarios (Non-IID friendly)
       and stricter masking in low-variance scenarios (Security focused).
    """
    ...
    # --- NOVELTY START: ADAPTIVE THRESHOLDING ---
    # Instead of hard-coding 80th percentile, we use statistical significance.
    # We keep weights that are > Mean + 1.0 * StdDev.
    # In 'noisy' (high variance) updates, this naturally selects fewer, stronger features.
    # In 'clean' updates, it selects more features.
    
    mu = np.mean(np.abs(server_vec))
    sigma = np.std(np.abs(server_vec))
    
    # Dynamic Threshold: We only trust gradients that are statistically significant
    threshold = mu + 1.0 * sigma
    
    mask = np.abs(server_vec) > threshold
    # --- NOVELTY END ---
    
    # Fallback: If mask is too aggressive (e.g. uniform distribution), keep top 10%
    if np.sum(mask) < (0.1 * len(server_vec)):
        threshold = np.percentile(np.abs(server_vec), 90)
        mask = np.abs(server_vec) > threshold
