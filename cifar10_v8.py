...
def compute_cluster_opinions(
    ...
    """
    FINAL FIX: Magnitude-Gated Gradient Alignment (MGGA).
    
    Logic:
    1. Focus ONLY on the final classification layer (net.15.weight).
    2. Check each class row individually.
    3. VETO TRIGGER: 
       If a client opposes the server (Negative Cosine) AND has a significant 
       update magnitude relative to the server (Ratio > 0.1), they are Malicious.
       
    Why this works:
    - Honest clients with missing data have random direction but SMALL magnitude. (Safe)
    - Attackers must use LARGE magnitude to flip labels. (Caught)
    """
    n_clients = len(updates)
    opinions = [None] * n_clients
    
    # Target the Final Linear Layer: shape [10, 128]
    # Rows 0-9 correspond to Classes 0-9
    target_layer = 'net.15.weight'
    
    # Server Reference Gradients (The Gold Standard)
    # Shape: (10, 128)
    server_rows = global_grad_ref[target_layer]
    server_norms = np.linalg.norm(server_rows, axis=1) + Config.EPS

    for i, upd in enumerate(updates):
        client_rows = upd[target_layer]
        client_norms = np.linalg.norm(client_rows, axis=1)
        
        # 1. Compute Per-Class Metrics
        # Cosine Similarity: Direction alignment
        dot_products = np.sum(server_rows * client_rows, axis=1)
        cos_sims = dot_products / (server_norms * client_norms + Config.EPS)
        
        # Magnitude Ratio: How strong is the client's update relative to server?
        # ratio = ||client_row|| / ||server_row||
        mag_ratios = client_norms / (server_norms + Config.EPS)
        
        # 2. The "Nuclear" Veto Logic
        is_malicious = False
        
        # We check specific classes. 
        # Label Flip (0->9) usually shows huge negative cosine on Class 0 (Source)
        # and Class 9 (Target).
        for c in range(10):
            # CONDITION: Opposing Direction AND Significant Magnitude
            # Thresholds:
            # Cosine < -0.2: Clearly pointing away (Random noise is usually -0.1 to 0.1)
            # Ratio > 0.1: Update is at least 10% of the server's expected strength
            if cos_sims[c] < -0.2 and mag_ratios[c] > 0.1:
                is_malicious = True
                break
        ...
        if is_malicious:
            # HARD PENALTY
            # b=0, d=0.9, u=0.1
            opinions[i] = {'b': 0.0, 'd': 0.9, 'u': 0.1}
        else:
            # 3. Honest Scoring (Reward helpfulness)
            # Only count classes where the client is actually helping (Cos > 0)
            positive_sims = cos_sims[cos_sims > 0]
            
            if len(positive_sims) > 0:
                # Average alignment on known classes
                score = np.mean(positive_sims)
                # Map to belief (0.0 to 0.9)
                b = score * 0.9
                u = 0.1
                d = 1.0 - b - u
            else:
                # Neutral/Useless (No classes aligned, but didn't trigger veto)
                b, d, u = 0.1, 0.1, 0.8
