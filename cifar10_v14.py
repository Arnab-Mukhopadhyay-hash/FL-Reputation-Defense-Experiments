...
class Config:
    ...
    # Hardware & System
    MAX_WORKERS = 2
    OUTPUT_DIR = "cifar10_v14_results"
    DEVICE_LIST = (
...
def compute_cluster_opinions(
    ...
    """
    NOVELTY: Soft-Calibration Trust (SCT).
    
    1. Full Dense Features: Uses raw gradients of Conv layers (net.0, net.4, net.8) 
       without masking, ensuring we capture all valid Non-IID signals.
    2. Sigmoid Calibration: Replaces the 'Hard Veto' with a smooth Sigmoid mapping.
       - Orthogonal (0.0) -> 0.5 Belief (Uncertainty). 
       - Opposite (-1.0) -> 0.0 Belief (Disbelief).
       This allows Benign Non-IID clients to survive while filtering attackers.
    """
    n_clients = len(updates)
    opinions = [None] * n_clients
    
    # Target Feature Extractors (Shared Vision)
    target_layers = ['net.0.weight', 'net.4.weight', 'net.8.weight']
    
    def get_concat_grad(grad_dict):
        tensors = [grad_dict[layer].flatten() for layer in target_layers]
        return np.concatenate(tensors)

    # 1. Server Reference (Full Dense Vector)
    # We REMOVE the masking to preserve Non-IID signals
    server_vec = get_concat_grad(global_grad_ref)
    server_norm = np.linalg.norm(server_vec) + 1e-10
    
    scores = []
    for i, upd in enumerate(updates):
        client_vec = get_concat_grad(upd)
        
        # 2. Compute Full Cosine Similarity
        client_norm = np.linalg.norm(client_vec) + 1e-10
        cosine_sim = np.dot(client_vec, server_vec) / (client_norm * server_norm)
        scores.append(cosine_sim)

    scores = np.array(scores)
    
    # 3. Map to Subjective Logic using Sigmoid Calibration
    # Scale factor 5.0 makes the transition sharp enough to punish -0.5 but soft enough for 0.0
    # Sigmoid(0) = 0.5 (Uncertainty)
    
    for i, score in enumerate(scores):
        # Sigmoid Function: 1 / (1 + e^(-k * score))
        # k=5 gives us:
        # score = -0.5 -> belief ~ 0.07 (High Disbelief)
        # score =  0.0 -> belief = 0.50 (High Uncertainty) -> Benign Non-IID survives!
        # score = +0.5 -> belief ~ 0.93 (High Belief)
        
        k = 5.0
        sigmoid_belief = 1 / (1 + np.exp(-k * score))
        
        # Map to Subjective Logic Tuple (b, d, u)
        # We reserve a small amount of mass for base uncertainty
        b = sigmoid_belief * 0.9
        u = 0.1
        d = max(0.0, 1.0 - b - u)
