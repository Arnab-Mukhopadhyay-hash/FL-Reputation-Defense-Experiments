...
class Config:
    ...
    # Hardware & System
    MAX_WORKERS = 2
    OUTPUT_DIR = "cifar10_v13_results"
    DEVICE_LIST = (
...
def compute_cluster_opinions(
    ...
    """
    NOVELTY: Deep Representation Alignment (DRA).
    
    1. Layer-Selective Trust: Ignores the noisy 'Classifier Head' (net.15) which 
       naturally diverges in Non-IID.
    2. Focuses on 'Feature Backbone' (net.0, net.4, net.8). Benign clients 
       align here (shared features). Malicious clients distort here (attack backprop).
    """
    n_clients = len(updates)
    opinions = [None] * n_clients
    
    # --- NOVELTY: Target the Feature Extractors, NOT the Classifier ---
    # We concatenate the weights of the 3 Convolutional Blocks
    target_layers = ['net.0.weight', 'net.4.weight', 'net.8.weight']
    
    # Helper to flatten and concatenate specific layers
    def get_concat_grad(grad_dict):
        tensors = [grad_dict[layer].flatten() for layer in target_layers]
        return np.concatenate(tensors)

    # 1. Server Reference (Deep Features)
    server_vec = get_concat_grad(global_grad_ref)
    
    # 2. Saliency Mask (Adaptive)
    # We focus on the top 20% of 'Feature' gradients where the server is learning shapes/textures.
    threshold = np.percentile(np.abs(server_vec), 80)
    mask = np.abs(server_vec) > threshold
    
    # Safety
    if np.sum(mask) == 0: mask[:] = True
    
    filtered_server = server_vec[mask]
    server_norm = np.linalg.norm(filtered_server) + 1e-10
    ...
    for i, upd in enumerate(updates):
        ...
        # 3. Compute Cosine Similarity on Deep Features
        client_norm = np.linalg.norm(filtered_client) + 1e-10
        cosine_sim = np.dot(filtered_client, filtered_server) / (client_norm * server_norm)
        
        scores.append(cosine_sim)

    scores = np.array(scores)
    
    # 4. Map to Subjective Logic
    for i, score in enumerate(scores):
        if score <= 0:
            # Attack on Features = Immediate Disbelief
            opinions[i] = {'b': 0.0, 'd': 1.0, 'u': 0.0}
        else:
            # High Alignment on Features = High Trust (even if labels differ)
            b = (score ** 2) * 0.95
            u = 0.05
            d = max(0.0, 1.0 - b - u)
