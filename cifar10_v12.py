...
class Config:
    ...
    # Hardware & System
    MAX_WORKERS = 2
    OUTPUT_DIR = "cifar10_v12_results"
    DEVICE_LIST = (
...
def compute_cluster_opinions(
    ...
    """
    NOVELTY: Conflict-Averse Trust Estimation (CATE).
    
    1. Focuses purely on 'Directional Conflict' rather than Magnitude or Similarity.
    2. Solves the Non-IID Orthogonality problem:
       - Benign Non-IID (Orthogonal) -> 0 Conflict -> High Trust.
       - Malicious (Opposite) -> High Conflict -> Low Trust.
    """
    ...
    # 2. Saliency Mask (Where is the Server Confident?)
    # We only penalize conflicts on weights the Server feels strongly about.
    # Using a fixed top-30% ensures we ignore background noise.
    threshold = np.percentile(np.abs(server_vec), 70)
    mask = np.abs(server_vec) > threshold
    
    filtered_server = server_vec[mask]
    
    scores = []
    for i, upd in enumerate(updates):
        client_vec = upd[target_layer].flatten()
        filtered_client = client_vec[mask]
        
        # 3. Calculate Element-wise Alignment
        # Positive = Agreement, Negative = Conflict, Zero = Neutral
        alignment = filtered_client * filtered_server
        
        # 4. Compute Conflict Ratio (Novel Metric)
        # We measure: How much of the client's 'energy' is spent contradicting the server?
        
        # Sum of absolute magnitude of CONFLICTING (negative) weights
        conflict_mass = np.sum(np.abs(alignment[alignment < 0]))
        
        # Sum of absolute magnitude of ALL active weights in the mask
        total_mass = np.sum(np.abs(alignment)) + 1e-10
        
        # Ratio: 0.0 means perfect alignment (or orthogonality). 1.0 means perfect conflict.
        conflict_ratio = conflict_mass / total_mass
        
        # Invert so 1.0 is Best, 0.0 is Worst
        score = 1.0 - conflict_ratio
        scores.append(score)

    scores = np.array(scores)
    
    # 5. Map to Subjective Logic
    # We use a strict mapping: Any significant conflict drops belief rapidly.
    for i, score in enumerate(scores):
        # Sigmoid-like non-linearity to push uncertain agents to 0.5
        # If score > 0.7 (Low conflict) -> High Belief
        # If score < 0.4 (High conflict) -> High Disbelief
        
        # Dynamic steepness based on round (harden over time)
        steepness = 10.0 if rnd > 5 else 5.0
        centered = score - 0.5
        sigmoid_score = 1 / (1 + np.exp(-steepness * centered))
        
        b = sigmoid_score * 0.95
        u = 0.05
        d = max(0.0, 1.0 - b - u)
