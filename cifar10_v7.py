...
def compute_cluster_opinions(
    ...
    """
    Nuclear Fix: Top-10% Sign Agreement Veto + Clustering
    """
    n_clients = len(updates)
    opinions = [None] * n_clients
    
    # --- FIX IS HERE: Changed 'net.12.weight' to 'net.15.weight' ---
    # net.12 is Flatten layer (no weights). net.15 is the final Linear layer.
    target_layer = 'net.15.weight' 
    
    server_grad = global_grad_ref[target_layer].flatten()
    
    # Identify the "Critical Weights": Top 10% largest gradients from Server
    k_top = int(0.1 * len(server_grad))
    top_indices = np.argsort(np.abs(server_grad))[::-1][:k_top]
    
    server_signs = np.sign(server_grad[top_indices])
    
    valid_indices = []  # Indices of clients that survive the veto
    
    for i, upd in enumerate(updates):
        client_grad = upd[target_layer].flatten()
        client_signs = np.sign(client_grad[top_indices])
        
        # Calculate Agreement Score
        agreement = np.mean(server_signs == client_signs)
        
        # HARD VETO: Threshold 0.55 handles Non-IID noise while catching flips
        if agreement < 0.55:
            # IMMEDIATE PENALTY
            opinions[i] = {'b': 0.0, 'd': 0.95, 'u': 0.05}
        else:
            valid_indices.append(i)

    # --- 2. Clustering for the Survivors ---
    if len(valid_indices) > 0:
        def flatten_all(upd):
            return np.concatenate([v.flatten() for v in upd.values()])

        X_valid = np.array([flatten_all(updates[i]) for i in valid_indices])
        
        # PCA Projection
        n_components = min(X_valid.shape[0], 10)
        if n_components > 0:
            pca = PCA(n_components=n_components)
            X_reduced = pca.fit_transform(X_valid)
            
            # Clustering
            current_k = min(n_clusters, len(valid_indices))
            if current_k > 1:
                kmeans = KMeans(n_clusters=current_k, n_init=10, random_state=Config.SEED)
                labels = kmeans.fit_predict(X_reduced)
                centroids = kmeans.cluster_centers_
            else:
                labels = np.zeros(len(valid_indices), dtype=int)
                centroids = np.mean(X_reduced, axis=0, keepdims=True)

            # Assign Opinions based on Clustering
            for loc_idx, glob_idx in enumerate(valid_indices):
                vec = X_reduced[loc_idx]
                c_idx = labels[loc_idx]
                
                dist = np.linalg.norm(vec - centroids[c_idx])
                avg_dist = np.mean([np.linalg.norm(vec - c) for c in centroids]) + Config.EPS
                
                # Belief decreases with distance from centroid
                belief = np.exp(-(dist**2) / (avg_dist**2))
                uncertainty = Config.W_FIXED
                disbelief = max(0.0, 1.0 - belief - uncertainty)
                
                opinions[glob_idx] = {'b': belief, 'd': disbelief, 'u': uncertainty}
        else:
             for idx in valid_indices:
                opinions[idx] = {'b': 0.5, 'd': 0.0, 'u': 0.5}
...
def run_experiment(
    ...
    with mp.Pool(processes=Config.MAX_WORKERS) as pool:
        for rnd in range(Config.ROUNDS):
            ...
            # --- Step A: Compute Server Reference Gradient ---
            ...
            # --- Step C: Reputation and Defense Logic ---
            
            # FIXED: Pass global_grad_ref here
