...
def run_experiment(partitions, malicious_flags, test_loader, zero_loader):
    ...
    for rnd in range(Config.ROUNDS):
        ...
        # 3. Calculate Scores (Cosine Similarity to Principal Direction)
        # We clamp at 0 because negative cosine means "opposite direction" (likely malicious)
        scores = np.array([max(0.0, cosine_similarity(u, principal_dir)) for u in flat_updates])
...
# 6. PLOTTING
# ======================================================
def plot_results(all_results, output_dir="cifar10_v3_results"):
    ...
    for dist in all_results:
        ...
        plt.title(f"Accuracy vs Rounds ({dist})")
        plt.xlabel("Rounds")
        plt.ylabel("Accuracy")
        plt.grid(True)
        plt.legend()
    ...
    for dist in all_results:
        ...
        plt.title(f"ASR vs Rounds ({dist})")
        plt.xlabel("Rounds")
        plt.ylabel("ASR (0->9)")
        plt.grid(True)
        plt.legend()
