...
def compute_cluster_opinions(
    ...
    n_clusters: int = 3,  # Unused
    outlier_threshold: float = 1.5, # Unused
    rnd: int = 0,
) -> list:
    """
    FINAL DEFENSE: Raw Projected Alignment (RPA).
    
    Logic:
    1. Extract the LAST LAYER (Classifier) gradients.
    2. Compute Raw Dot Product between Client and Server Ref.
    3. NO COSINE NORMALIZATION. Magnitude is the key discriminator.
    4. If Dot Product <= 0: The client is actively opposing the server -> VETO.
    5. If Dot Product > 0: The client is helping -> REWARD.
    """
    n_clients = len(updates)
    opinions = [None] * n_clients
    
    # Target the Final Linear Layer (Most sensitive to label flips)
    target_layer = 'net.15.weight'
    
    # Flatten Server Reference for the target layer
    ...
    for i, upd in enumerate(updates):
        ...
        # Compute Raw Dot Product (Projection)
        # Malicious (Label Flip) -> Large Magnitude * Opposite Direction -> Large Negative Score
        # Benign (Missing Data) -> Small Magnitude * Random Direction -> Small Score (near 0)
        score = np.dot(client_vec, server_vec)
        scores.append(score)

    scores = np.array(scores)
    
    # Normalize purely for the belief mapping (0 to 1), relative to the best performer
    # This does NOT affect the Veto logic.
    max_score = np.max(scores) + Config.EPS
    
    for i, score in enumerate(scores):
        if score <= 0:
            # OPPOSITION: Immediate Veto
            # Belief=0, Disbelief=1.0, Uncertainty=0.0
            opinions[i] = {'b': 0.0, 'd': 1.0, 'u': 0.0}
        else:
            # ALIGNMENT: Reward based on contribution strength relative to the best client
            normalized_score = score / max_score
            
            # Map to Subjective Logic
            # Cap belief at 0.9 to ensure system remains adaptive
            b = float(normalized_score) * 0.9  
            u = 0.1
            d = max(0.0, 1.0 - b - u)
